...
demo_migration() {
    ...
    print_info "Note: Migration to real backends is NOT YET IMPLEMENTED."
    echo "The migrate command is scaffolded for future use."
    echo ""
    echo "Current status: Real backend migration returns error:"
    echo "  'Error: real [backend] backend not yet implemented for migration'"
    echo ""
    echo "Future commands (when implemented):"
    echo "  todoat migrate --from sqlite --to nextcloud           - Migrate all"
    echo "  todoat migrate --from sqlite --to nextcloud --dry-run - Preview migration"
    echo "  todoat migrate --target-info nextcloud --list Work    - Check target"
}
