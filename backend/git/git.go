...
func (b *Backend) loadFile() error {
	...
	for scanner.Scan() {
		...
		if matches := taskPattern.FindStringSubmatch(line); len(matches) == 4 && currentList != nil {
			...
			summary, priority, dueDate, categories := parseTaskText(taskText)
...
func (b *Backend) saveFile() error {
	...
	for _, list := range b.lists {
		...
		rootTasks, childrenMap := organizeTasksHierarchically(b.tasks[list.ID])

		// Write tasks with proper indentation
		for _, task := range rootTasks {
			writeTaskTree(&sb, &task, childrenMap, 0)
		}
...
// writeTaskTree writes a task and its children with proper indentation
func writeTaskTree(sb *strings.Builder, task *backend.Task, childrenMap map[string][]backend.Task, level int) {
	...
	if children, ok := childrenMap[task.ID]; ok {
		for _, child := range children {
			writeTaskTree(sb, &child, childrenMap, level+1)
		}
	}
}

// parseStatusChar converts markdown checkbox to TaskStatus
...
func parseTaskText(text string) (summary string, priority int, dueDate *time.Time, categories string) {
	...
	// Extract due date: @2024-01-15 or @tomorrow
